...
public class GuardedAvlGTree<K, V> extends AvlGTree<K, V> {
    ...
    public V put (final K key, final V val) {
        ...
        if (shouldWarn && ret != correctVal) // Object equality is sufficient
            System.out.println("My map said " + ret + " instead of " + correctVal);
...
//       // shouldWarn = false;
//        final V correctVal = guard.remove(key);
//        final V myVal = super.remove(key);
//        if(shouldWarn && !myVal.equals(correctVal))
//        	System.out.println("My map said " + myVal + 
//        			" instead of " + correctVal);
//        return correctVal; // Save yourself
    }
    @Override
    public int size() {
