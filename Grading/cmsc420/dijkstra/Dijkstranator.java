 ...
 * 
 * Copyright David Renie (University of Maryland, College Park), 2005 Copyright
 * Ben Zoller (University of Maryland, College Park), 2006 All rights reserved.
 * Permission is granted for use and modification in CMSC420 at the University
 * of Maryland.
 */
...
public class Dijkstranator {
    ...
    public Path getShortestPath(final City startCity, final City endCity) {
        ...
        if (roads.getNumberOfCities() > 0) {
             ...
             */
            unsettledCities = new PriorityQueue<DijkstraCity>(roads.getNumberOfCities());
        } else {
            /*
        ...
        final TreeMap<City, City> previousCity = new TreeMap<City, City>(new CityNameComparator());

        /*
         * the best estimate of the shortest distance from the source to each
         * vertex
         */
        final TreeMap<City, Double> shortestDistanceFound = new TreeMap<City, Double>(new CityNameComparator());
        ...
        while (!unsettledCities.isEmpty()) {
            ...
            if (!settledCities.containsKey(cityToSettleName)) {
                ...
                for (Road road : roads.getRoadSet(cityToSettle.getCity())) {
                    ...
                    final City adjacentCity = road.getOtherCity(cityToSettleName);
                    ...
                    if (!settledCities.containsKey(adjacentCityName)) {
                         ...
                         */
                        final double adjacentCityDistance = shortestDistanceFound.get(adjacentCity);
                        final double distanceViaCityToSettle = shortestDistanceFound.get(cityToSettle.getCity())
                                + road.getDistance();
                        ...
                        if (adjacentCityDistance > distanceViaCityToSettle) {
                             ...
                             */
                            shortestDistanceFound.put(adjacentCity, distanceViaCityToSettle);

                            /* update the other data structures */
                            previousCity.put(adjacentCity, cityToSettle.getCity());
                            unsettledCities.offer(new DijkstraCity(adjacentCity, distanceViaCityToSettle));
                        }
                    }
                }
            }
