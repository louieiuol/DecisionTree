 ...
 * 
 * Copyright Ben Zoller (University of Maryland, College Park), 2007 Copyright
 * David Renie (University of Maryland, College Park), 2005 All rights reserved.
...
public class RoadAdjacencyList {
     ...
     * @param road
     *            road to be checked
     * @return <code>true</code> if the road is in the list, <code>false</code>
     *         otherwise
     */
     ...
     */
    public Set<Road> removeRoadsForCity(final City city) {
        Set<Road> roadsForCity = adjacencyList.get(city);

        if (roadsForCity != null) {
            adjacencyList.remove(city);
        } else roadsForCity = Collections.<Road>emptySet();
    ...
    /*returns a set of cities for a specific Z*/
    public Set<City> getCitySet(int z){
    	Set<City> toRet = new TreeSet<City>(new CityNameComparator());
    	for(City c : adjacencyList.keySet()){
    		if(c.getZ() == z && !c.isPortal()){
    			toRet.add(c);
    		}
    	}
    	return toRet;
    ...
    public String toString() {
        ...
        for (final City city : adjacencyList.keySet()) {
            ...
            s.append(cityName);
            s.append(":\n");
            for (Road r : adjacencyList.get(cityName)) {
