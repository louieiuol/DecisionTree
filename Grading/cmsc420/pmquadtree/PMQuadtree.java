...
import cmsc420.city.VectorUtils;
import cmsc420.geom.Inclusive2DIntersectionVerifier;
import cmsc420.portal.Portal;
import cmsc420.portal.PortalNameComparator;
...
public abstract class PMQuadtree {
	...
	final protected TreeSet<City> portalsInPMQuadtree;
	...
	public abstract class Node {
		...
		public Node add(final Road g, final Point2D.Float origin,
				...
				throws InvalidPartitionThrowable, IntersectingRoadsThrowable {
        ...
        public Point2D.Double getContentsMean() {
            return null;
        }
	}
	...
	public class White extends Node {
		...
		public Node add(final Road g, final Point2D.Float origin,
				final int width, final int height)
				throws IntersectingRoadsThrowable, InvalidPartitionThrowable {
	...
	public class Black extends Node {
		...
		public Node add(final Road g, final Point2D.Float origin,
			...
			if (!g.isIsolatedCity()) {
				...
				if (Inclusive2DIntersectionVerifier.intersects(g.getStart()
						.toPoint2D(), rect)) {
					addRoadToList(new Road(g.getStart(), g.getStart()));
				}
	
				/* check if end point intersects with region */
				if (Inclusive2DIntersectionVerifier.intersects(g.getEnd()
						.toPoint2D(), rect)) {
					addRoadToList(new Road(g.getEnd(), g.getEnd()));
				}

				/* check for road intersection */
				for (int i = numPoints; i < geometry.size(); i++) {
					Road road = geometry.get(i);

					/*
					 * check for road intersection
					 */
					if (Road.intersects(road, g)) {
						throw new IntersectingRoadsThrowable(road
								.getCityNameString());
					}
				}
		...
		private Node partition(final Point2D.Float origin, final int width,
				final int height) throws InvalidPartitionThrowable,
				IntersectingRoadsThrowable {
        ...
        public Point2D.Double getContentsMean() {
            return getCity() == null ? null : getCity().toPoint2D();
        }
	}
	...
	public class Gray extends Node {
		...
		public Node add(final Road g, final Point2D.Float origin,
				final int width, final int height)
				throws InvalidPartitionThrowable, IntersectingRoadsThrowable {
			boolean invalidPartition = false;
			boolean intersectingRoads = false;
			count = 0;
			mean = new Point2D.Double();
			...
			for (int i = 0; i < 4; i++) {
				...
				if (Inclusive2DIntersectionVerifier.intersects(g.toLine2D(),
					...
					try {
						...
						intersectingRoads = true;
					}
				}
				if (children[i].getContentsMean() != null
				        && regions[i].contains(children[i].getContentsMean())) {
                    count += children[i].getContentsCount();
                    VectorUtils.addVector(mean, VectorUtils.scaleVector(children[i].getContentsMean(),
                            children[i].getContentsCount()));
				}
			}
			if (count > 0)
			    VectorUtils.scaleVector(mean, 1.0/count);
			
			if (intersectingRoads) {
				throw new IntersectingRoadsThrowable();
			}
		...
		public Node remove(Road g, Point2D.Float origin, int width, int height) {
			...
			// pause: avoid numerical instability; recalculate count and mean
			for (int i = 0; i < 4; i++) {
                if (children[i].getContentsMean() != null
                        && regions[i].contains(children[i].getContentsMean())) {
                    count += children[i].getContentsCount();
	...
	public PMQuadtree(final Validator validator, final int spatialWidth,
		...
		portalsInPMQuadtree = new TreeSet<City>(new CityNameComparator());
	...
	//-------------------------------------------//
	//PORTAL METHODS
	public void addPortal(final City c) throws InvalidPartitionThrowable, RoadAlreadyExistsThrowable, IsolatedCityAlreadyExistsThrowable, IsolatedCityOutOfSpatialBoundsThrowable, IntersectingRoadsThrowable{
		//adds it to our treeSet of portals, adds to quadTree as city
		portalsInPMQuadtree.add(c);
		add(c);
	}
	
	//removes a portal
	public void removePortal(final String name) throws IsolatedCityNotMappedThrowable{
		
		City c = getPortal(name);
		portalsInPMQuadtree.remove(c);
		remove(c);
	}
	
	public City getPortal(final String name){
		for(City c : portalsInPMQuadtree){
	...
	//checks if portal p exists
	public boolean containsPortal(final City c){
		return portalsInPMQuadtree.contains(c);
	}
	
	//checks if a portal with that name exists
	public boolean containsPortalByName(final String name){
		for(City c : portalsInPMQuadtree){
			if(c.getName().equals(name)){
				return true;
			}
	...
	//checks if a portal at that coordinate exists
	public boolean containsPortalByCoordinate(int x, int y){
		for(City p : portalsInPMQuadtree){
	...
	//---------------------------------------------//
	public void add(final Road g) throws RoadAlreadyExistsThrowable,
			IsolatedCityAlreadyExistsThrowable, InvalidPartitionThrowable,
			IntersectingRoadsThrowable, RoadOutOfSpatialBoundsThrowable {

		/* check bounds */
		Rectangle2D.Float bounds = new Rectangle2D.Float(0, 0, spatialWidth,
				spatialHeight);
		if (!Inclusive2DIntersectionVerifier.intersects(g.getStart()
				.toPoint2D(), bounds)
	...
	public void add(final City c) throws RoadAlreadyExistsThrowable, IsolatedCityAlreadyExistsThrowable,
			InvalidPartitionThrowable, IsolatedCityOutOfSpatialBoundsThrowable, IntersectingRoadsThrowable {
		Road isoCity = new Road(c);

		/* check bounds */
		Rectangle2D.Float bounds = new Rectangle2D.Float(0, 0, spatialWidth,
				spatialHeight);
		if (!Inclusive2DIntersectionVerifier.intersects(isoCity.getStart()
				.toPoint2D(), bounds)) {
		...
		try {
			...
			cleanupErrorForIsolatedCity(isoCity);
			throw e;
		} catch (IntersectingRoadsThrowable e) {
	...
	private void cleanupError(final Road g) {
		increaseNumRoadsMap(g.getStart().getName());
		increaseNumRoadsMap(g.getEnd().getName());
	...
	private void cleanupErrorForIsolatedCity(final Road c) {
